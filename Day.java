...
/*
 * Write a program that computes the number of days that have elapsed since you were born. 
...
public class Day
{
	public static void main(String args[])
	{
		...
		//while loop uses flag that is only set to false when program finishes successfully, so any error will cause loop to retry
		while (flag)
		{
			try
			{
				System.out.println("Enter the day you were born in the format mm/dd/yyyy:");
				String birthday = input.nextLine();
				Day birth = new Day(Integer.parseInt(birthday.substring(6, 10)), Integer.parseInt(birthday.substring(0, 2)), Integer.parseInt(birthday.substring(3, 5)));
				
				//LocalDate.now() provides a LocalDate object of todays date in the format yyyy/mm/dd
				String now = java.time.LocalDate.now().toString(); 
				Day today = new Day(Integer.parseInt(now.substring(0, 4)), Integer.parseInt(now.substring(5, 7)), Integer.parseInt(now.substring(8, 10)));
				
				System.out.println("\nThere have been " + today.daysFrom(birth) + " days since you were born.");
				flag = false;
